...
| --- | --- |
| `fast` | 3–4 мин |
| `full` (холодный кеш, Testcontainers) | 8–10 мин |
| Задание агента | 3–5 мин |

Раунд ≈ 15 мин, типовая задача в 2–3 раунда ≈ 35–45 мин → **≈ 45–55 задач в месяц**
на бесплатном лимите. Отсюда `needs: fast` у `full`: сломанный раунд стоит 4 минуты,
а не 14.
...
- Раннер в Docker-контейнере на VPS, `runs-on: self-hosted`. **Только на приватном
  репозитории**: на публичном PR из форка выполнит произвольный код на твоей машине.
