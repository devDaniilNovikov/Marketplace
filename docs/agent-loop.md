...
## 9. ArchUnit: гейт работает

`archunit_store` закоммичен, `allowStoreCreation=false`. Пропажа стора — ошибка
сборки, новое нарушение изоляции валит CI.

Так было не всегда: стор не коммитился, `allowStoreCreation=true`, и каждый чекаут
создавал его заново, замораживая в том числе новые нарушения. Правило не падало
никогда. Вдобавок ArchUnit клал стор относительно рабочего каталога JVM тестов,
то есть в корень проекта, а не в ресурсы — путь теперь задан явно строкой
`freeze.store.default.path`.

В сторе восемь нарушений, все — задача **B1**: сущности и репозиторий лежат
в публичном `api` и объявлены `public`. Список должен только уменьшаться.

Перегенерация после исправления: **Actions → ArchUnit Store → Run workflow**.
Обязательна: при `allowStoreUpdate=true` прогон уменьшает стор в своей рабочей
копии, и без коммита изменение теряется.

Отдельно: `MarketplaceApplicationTests` помечен `@Disabled` до задач B1–B3. Значит
«CI зелёный» **не означает**, что контекст Spring поднимается. Держи
