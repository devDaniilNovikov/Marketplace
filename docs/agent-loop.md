...
| Джоб | Оценка | Факт, прогон №1 |
| --- | --- | --- |
| `fast` | 3–4 мин | **1 мин** |
| `full` (холодный кеш, Testcontainers) | 8–10 мин | **1 мин 25 с**, но тесты упали рано — на зелёном будет больше |
| Задание агента | 3–5 мин | не измерялось |

Оценки оказались пессимистичными: холодный `full` с подъёмом Postgres уложился
в полторы минуты. Пересчитать бюджет минут стоит после первого зелёного прогона,
а не по этим числам.
